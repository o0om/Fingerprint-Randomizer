...
function loadConfig() {
  chrome.storage.sync.get([
    ...
    'timezone', 'geolocation', 'extensionmask', 'isolatedstorage', 'useragent', 'referrer',
    ...
    const toggleMap = {
      ...
      'timezone': result.timezone || false,
      'geolocation': result.geolocation || false,
      'extensionmask': result.extensionmask || false,
      'isolatedstorage': result.isolatedstorage || false,
      'useragent': result.useragent || false,
      'referrer': result.referrer || false,
    ...
    window._currentProfile = result.currentProfile || 'strict';
...
function updateStatus() {
  ...
  const activeCount = toggles.filter(key => {
    const el = document.getElementById(`toggle-${key}`);
...
function applyProfile(profile) {
  const profiles = {
    strict: {
      ...
      timezone: true, geolocation: true, extensionmask: true, isolatedstorage: true, useragent: true, referrer: true,
      stun: true, turn: true, ml: true, fonts: true, behavioral: true
    },
    balanced: {
      fingerprint: true, webgl: true, audio: true, webrtc: true, canvas: true, microphone: true,
      timezone: false, geolocation: false, extensionmask: false, isolatedstorage: false, useragent: false, referrer: false,
  ...
  if (profile === 'manual') {
    ...
    logActivity('PROFILE MANUAL SELECTED (NO CHANGES APPLIED)');
    updateStatus();
    return;
  }
